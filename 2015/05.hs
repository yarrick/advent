...
nicestr :: [String] -> Int
nicestr str = length $ filter notCensored $ filter doubleLetter $ filter (minVowels 3) str
...
nicestr2 :: [String] -> Int
nicestr2 str = length $ filter doubleLetterSpaced $ filter doublePair str

process rows = map show [nicestr rows, nicestr2 rows]
